async function sendCommandToTab(command, tab) {
  ...
  async function findAndClick(command) {
    ...
    function usingVolumeSlider(command) {
      usingSlider(
        "[data-testid*=volume] input[type=range]",
        command == "volume-up"
      );
    }
    ...
    function usingSelector(command) {
      ...
      const selectors = {
        ...
        // like: [
        //   ".OgkbKIVYE_mrNpYESylB.Button-sc-1dqy6lx-0.leSyTc.emaScS.",
        //   `[aria-label="お気に入りの曲に追加"]`,
        // ],
        like: [
          "footer .Button-sc-1dqy6lx-0",
          `[aria-label="お気に入りの曲に追加"]`,
        ],
      }[command];
      if (!selectors) throw "";
    ...
    function usingSvg(command) {
      ...
      while (!!e && !!e.tagName && e.tagName.toLowerCase() !== tag)
        e = e.parentNode;
      clickAndAnimate(e);
    }
    ...
    function dispatchCommand(command) {
      if (command == "volume-up" || command == "volume-down") {
        try {
          ...
          console.warn(
            `[Spotify Web Player Hotkeys] Could not change volume slider: ${e}`
          );
        }
        return;
      }
        ...
        try {
          ...
          if (command == "seek-forward" || command == "seek-backward") {
            ...
            try {
              ...
              console.warn(
                `[Spotify Web Player Hotkeys] Could not change seek slider: ${e}`
              );
            }
          }
          console.warn(
            `[Spotify Web Player Hotkeys] Could not click '${command}': ${e}`
          );
        }
      }
    }
    ...
    const result = (
      await import(
        chrome.runtime.getURL("web_accessible/checkPageInteraction.js")
      )
    ).default(command, dispatchCommand);
    if (result === "requestUserInteraction") {
      console.warn(
        "[Spotify Web Player Hotkeys] Page interaction required before playback"
      );
      return result;
    }
...
async function getSpotifyTab(
  ...
  if (!spotifyTab || !spotifyTab.url?.startsWith("https://open.spotify.com/"))
    [spotifyTab] = await chrome.tabs.query({
  ...
  if (createIfNotExist && !spotifyTab) {
    ...
    lastNormalWinId ??= (
      await chrome.windows.create({ focused: false, incognito: false })
    ).id;
    spotifyTab = await chrome.tabs.create({
      url: "https://open.spotify.com",
      active: false,
      windowId: lastNormalWinId,
    });
  }
  ...
  if (spotifyTab)
    spotifyTab.focusedAndActive =
      spotifyTab.active &&
      (await chrome.windows.get(spotifyTab.windowId))?.focused;
...
chrome.commands.onCommand.addListener(async function (command) {
  ...
  if (response?.[0]?.result === "requestUserInteraction")
    await getSpotifyTab(false, true);
});
...
let retrievingState = chrome.storage.session.get("state").then(async (data) => {
...
chrome.windows.onFocusChanged.addListener(async (winId) => {
  if (
    ...
    (await chrome.windows.get(winId))?.incognito
  )
    return;
